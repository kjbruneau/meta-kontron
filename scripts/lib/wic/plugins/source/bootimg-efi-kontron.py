...
#
# DESCRIPTION
# This implements the slightly customized 'bootimg-efi' source plugin class
# for 'wic'
#
# AUTHORS
# Tom Zanussi <tom.zanussi (at] linux.intel.com>
# Mikhail Vanyulin <mvanyulin (at] dev.rtsoft.ru>
#
...
#        loader_conf += "default boot\n"
        ...
        if not custom_cfg:
            ...
            title = source_params.get('title2')

            boot_conf = ""
            boot_conf += "title %s\n" % (title if title else "boot")
            boot_conf += "linux /%s\n" % kernel

            label = source_params.get('label2')
            ...
            title = source_params.get('title1')

            install_conf = ""
            install_conf += "title %s\n" % (title if title else "boot")
            install_conf += "linux /%s\n" % kernel

            label = source_params.get('label1')
            label_conf = "LABEL=Boot root=%s" % creator.rootdev
            if label:
                label_conf = "LABEL=%s" % label

            install_conf += "options %s %s\n" % \
                             (label_conf, bootloader.append)

            if initrd:
                initrds = initrd.split(';')
                for rd in initrds:
                    install_conf += "initrd /%s\n" % rd
        ...
        logger.debug("Writing systemd-boot config "
                     "%s/hdd/boot/loader/entries/install.conf", cr_workdir)
        cfg = open("%s/hdd/boot/loader/entries/install.conf" % cr_workdir, "w")
        cfg.write(install_conf)
        cfg.close()
